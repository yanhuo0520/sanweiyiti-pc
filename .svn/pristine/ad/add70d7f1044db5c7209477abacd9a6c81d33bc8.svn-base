<template>
    <div class="order-list"  v-loading="loading">
		...
		<el-alert style="margin-bottom: 10px" title="因农行延迟到账原因,若提现失败,请在订单【支付15分钟之后进行退款】操作" type="warning"></el-alert>
		<div class="table-con">
				...
				<el-table :data="data" border >
					...
					<el-table-column prop="contents" label="订单状态"  width="160" align="center">
						<template slot-scope="scope">
							...
							<el-tag v-if="scope.row.order_status == 2" type="" effect="dark">待发货</el-tag>
							<el-tag v-if="scope.row.order_status == 3" type="info" >支付失败</el-tag>
							<el-tag v-if="scope.row.order_status == 4" type="info" >待支付</el-tag>
							<el-tag v-if="scope.row.order_status == 5" type="info" >已取消</el-tag>
							<el-tag v-if="scope.row.order_status == 6" type="info" >已退款</el-tag>
							<el-tag v-if="scope.row.order_status == 7" type="info" >待收货</el-tag>
							<el-tag v-if="scope.row.order_status == 8" type="warning" effect="dark">退换货</el-tag>
							<el-tag v-if="scope.row.order_status == 9" type="info" >待评价</el-tag>
					...
					<el-table-column label="操作" width="160" fixed="right" align="center">
						<template slot-scope="scope">
							...
							<el-button  v-if="scope.row.order_status == 8 && isRefund"  size="small" plain type="warning" @click="orderRefund(scope.row,scope.$index)" :loading="orderRefundLoading">退款</el-button>
						</template>
					</el-table-column>
				</el-table>
			</template>
