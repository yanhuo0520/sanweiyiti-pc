            ...
            <div>
              <el-table :data="tableData" v-if="tableData && tableData.length > 0" border>
                ...
                <el-table-column prop="log_date" label="操作" align="center" width="160" v-if="isEdit || isDel">
                  <template slot-scope="scope">
                      <el-button v-if="isEdit" type="primary" plain size="small" @click="edit(scope.row.cate_id,scope.row.cate_name,scope.row.icon,scope.row.is_mobile_show,scope.row.is_recommend,scope.row.real_name)" >编辑</el-button>
                      <el-button v-if="isDel" type="danger" plain size="small" @click="del(scope.row.cate_id)" >删除</el-button>
                  </template>
                </el-table-column> -->
              </el-table>
