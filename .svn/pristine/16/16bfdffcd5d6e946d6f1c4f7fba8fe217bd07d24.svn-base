<template>
    <div class="passbook-term-list" v-loading="loading">
		...
		<el-dialog :title="capitalType == 1 ? '添加互助金利率' : '编辑互助金利率'" :visible.sync="ruleFormDialog" class="product-dialog">
			<el-form  ref="ruleForm"  :model="ruleForm" :rules="capitalRules" label-width="100px" class="product-form clearfix" >
				<el-form-item label="存入类型:" prop="type_id">
					<el-select v-model="ruleForm.type_id" placeholder="请选择存入类型" @change="capitalTypeChange"  :disabled="capitalType == 2">
				...
				<el-form-item label="存款期限标识:" prop="sign" v-if="ruleForm.type_id == 2">
					<el-select v-model="ruleForm.sign" placeholder="请选择存款期限标识" :disabled="capitalType == 2">
				...
				<el-form-item label="存款期限" prop="number"  v-if="ruleForm.type_id == 2" >
					<el-input v-model="ruleForm.number" placeholder="请输入数字" :disabled="capitalType == 2"></el-input>
				</el-form-item>
...
export default {
  ...
  methods: {
	...
	// 选择存入类型
	capitalTypeChange(val) { // 2-定期 3-活期
		if(val == 2) {
			this.ruleForm = {
                rate: '',
                number: '',
