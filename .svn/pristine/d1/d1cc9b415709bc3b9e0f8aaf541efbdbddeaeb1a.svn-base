            ...
            <template v-if="data && data.length > 0">
                <el-table :data="data" border >
                    ...
                    <el-table-column label="操作" align="center" v-if="isEdit || isDel">
                        <template slot-scope="scope">
                            <el-button type="primary" plain size="small"   @click="editBanner(scope.row)" v-if="isEdit">编辑</el-button>
                            <el-button type="danger" plain size="small"  @click="delBanner(scope.row,scope.$index)" v-if="isDel">删除</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </template>
        ...
        <el-dialog :title="'农资商城'+(picForm.id ? '编辑' : '添加') + '轮播图'" :visible.sync="picDialog" :close-on-click-modal="false" :close-on-press-escape="false">
			<el-form ref="picForm" :model="picForm" :rules="picRules" label-width="80px">
				...
				<el-form-item label="链接ID" prop="c_id">
					<el-input v-model="picForm.c_id" placeholder="请输入ID"></el-input>
				</el-form-item>
			</el-form>
