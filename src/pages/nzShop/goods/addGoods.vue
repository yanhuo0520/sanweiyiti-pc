  ...
  <div class="addGoods" v-loading="loading">
    <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="150px">
      <div class="breadcrumb-con">
        ...
        <div class="breadcrumb-info">
          <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item class="breadcrumb-tit">
              {{$route.query.tag == 'approve' ? '商品详情' : ($route.query.tag == 'edit' ? '编辑商品':'添加商品')}}
            </el-breadcrumb-item>
          </el-breadcrumb>
        </div>
      ...
      <div class="form-item-con clearfix">
        <el-form-item label="商品名称:" prop="goods_name">
          <el-input v-model="ruleForm.goods_name" placeholder="请输入商品名称" clearable :readonly="goods_id && !isEdit">
          </el-input>
        </el-form-item>
        <el-form-item label="助记码:">
          <el-input v-model="ruleForm.simple_code" placeholder="请输入助记码" clearable :readonly="goods_id && !isEdit">
          </el-input>
        </el-form-item>
        ...
        <el-form-item label="分类:" prop="assort_id">
          <el-select v-model="ruleForm.assort_id" placeholder="请选择一级分类" :disabled="goods_id && !isEdit"
            @change="changeCate">
        ...
        <el-form-item label="商品缩略图:" prop="thumb" style="height:88px;">
          ...
          <el-upload class="avatar-uploader" action="http://coopera.xfd365.com/user/auth/uploadImg"
            :on-preview="handleThumbPreview" :on-remove="handleThumbRemove" :on-success='handlePic'
            :before-upload="beforeAvatarUpload" list-type="picture-card" :show-file-list='true' :limit="1"
            :file-list='thumbs'>
        ...
        <el-form-item label="商品视频:">
          <el-upload class="avatar-uploader" action="http://coopera.xfd365.com/user/auth/uploadImg"
            :show-file-list="false" :on-success="handleVideo" :on-error="uploadErr" :disabled="goods_id && !isEdit">
            <video v-if="ruleForm.video_url" :src="ruleForm.video_url" class="avatar" autoplay controls></video>
            <i v-else class="el-icon-plus avatar-uploader-icon"></i>
          </el-upload>
        </el-form-item>
        <el-form-item label="商品规格:">
          <p @click="isAdd = true" :class="{'color-blue': specContent == '点击填写商品规格'}" v-if="specContent == '点击填写商品规格'">
            {{specContent}}</p>
          <p v-else>{{specContent}}</p>
        </el-form-item>
        <el-form-item label="生产日期:">
          <el-date-picker v-model="ruleForm.production_date" type="date" placeholder="选择生产日期" format="yyyy-MM-dd"
            value-format="yyyy-MM-dd" :disabled="goods_id && !isEdit">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="保质期:">
          <el-input v-model="ruleForm.guarantee_period" placeholder="请输入保质期" clearable :readonly="goods_id && !isEdit">
          </el-input>
          <span>天(-1为不过期)</span>
        </el-form-item>
        <el-form-item label="库存警告:">
          <el-input v-model="ruleForm.storage_warning" placeholder="请输入库存警告" clearable :readonly="goods_id && !isEdit">
          </el-input>
          <span>-1为不警告</span>
        </el-form-item>
        <el-form-item label="商品相册:">
          <el-upload class="avatar-uploader" action="http://coopera.xfd365.com/user/auth/uploadImg"
            ...
            list-type="picture-card" :show-file-list='true' :file-list='imageUrls'>
        ...
        <el-form-item label="预售时间:" v-if="yushouRadio==1 && assort_name == '农资商城'">
          <el-date-picker v-model="ruleForm.booking_date" type="date" placeholder="选择预售时间"
        ...
        <el-form-item label="发货时间:" v-if="yushouRadio==1 && assort_name == '农资商城'">
          <el-date-picker v-model="ruleForm.fahuo_date" format="yyyy-MM-dd" value-format="yyyy-MM-dd" type="daterange"
            range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="pickerOptions1">
          </el-date-picker>
        </el-form-item>
      ...
      <el-table :data="specArr" style="width: 100%" class="specData"
        v-if="($route.query.tag=='edit' || $route.query.tag=='approve') && specArr.length != 0">
        ...
        <el-table-column width="180" label="阶梯价格" v-if="assort_name == '农资商城'">
          <template slot-scope="scope">
            <div
              v-if="(ruleForm.spcifi[scope.$index].tiered[0] || {}).status == 0 || ruleForm.spcifi[scope.$index].tiered.length == 0">
        ...
        <el-table-column prop="pic" label="图片" width="180">
          <template slot-scope="scope">
            <el-upload class="avatar-uploader" action="http://coopera.xfd365.com/user/auth/uploadImg"
              ...
              :before-upload="beforeAvatarUpload" :on-error="uploadErr" :disabled="goods_id && !isEdit">
        ...
        <el-table-column prop="retail_price" label="价格">
          <template slot-scope="scope">
            <el-input v-model="scope.row.retail_price" :readonly="goods_id && !isEdit"
              @input="scope.row.retail_price = utils.handlePrice(scope.row.retail_price)"></el-input>
          </template>
        </el-table-column>
        <el-table-column prop="purchase_price" label="成本价">
          <template slot-scope="scope">
            <el-input v-model="scope.row.purchase_price" :readonly="goods_id && !isEdit"
              @input="scope.row.purchase_price = utils.handlePrice(scope.row.purchase_price)"></el-input>
          </template>
        </el-table-column>
        <el-table-column prop="discount_price" label="指导价">
          <template slot-scope="scope">
            <el-input v-model="scope.row.discount_price" :readonly="goods_id && !isEdit"
              @input="scope.row.discount_price = utils.handlePrice(scope.row.discount_price)"></el-input>
          </template>
        </el-table-column>
        <!-- <el-table-column prop="team_owner_money" label="分销金额">
          <template slot-scope="scope">
            <el-input v-model="scope.row.team_owner_money" :readonly="goods_id && !isEdit"
              @input="scope.row.team_owner_money = utils.handlePrice(scope.row.team_owner_money)"></el-input>
          </template>
        </el-table-column> -->
        <el-table-column prop="storage" label="库存">
          <template slot-scope="scope">
            <el-input v-model="scope.row.storage" :readonly="goods_id && !isEdit"
              @input="scope.row.storage = scope.row.storage.replace(/\D/g,'')"></el-input>
          </template>
        </el-table-column>
        <el-table-column prop="self_code" label="自编码">
          <template slot-scope="scope">
      ...
      <el-button type="primary" @click="addGuige"
        v-if="($route.query.tag=='edit' || $route.query.tag=='approve') && specArr.length != 0"
        style="display:block;margin: 20px auto;">添加规格</el-button>

      <div id="editor-con">
        <div :id="editorId" style="width:100%;height:300px"></div>
      </div>

      <el-button v-if="isEdit" type="primary" @click="submitForm('ruleForm')" :loading="submitLoading" class="btn-con">
      ...
      <div class="approve-btn-con" v-if="lastPath == '/goodsApproveList' && isApprove">
        ...
        <template v-else>
          <el-button type="primary" round>
            {{ruleForm.check_status == 1 ? '已同意' : (ruleForm.check_status == 2 ? '已拒绝' : '审核中') }}</el-button>
        </template>
      </div>
    </el-form>

    <el-dialog :visible.sync="isAdd" title="添加多规格" class="wid80">
      <div class="specification">
        如何添加多规格商品: <span style="color:#00CD92;cursor:pointer;" @click="isFanli=true;">范例和说明 <img
            style="width:15px;height:15px;" src="http://sc.xfd365.com/images/info.png"></span>
        <p>商品规格</p>
        <div class="goodsNorm">
          <div v-for="(item,index) in specs" :key="index" class="goodsInput">
            ...
            <div class="goodsImg">
              <div v-for="(itm,idx) in item.children" :key="idx">
                <p>
                  <input type="text" class="el-input__inner" v-model="itm.name" placeholder="请输入内容"
                    @input="propertiesInput">
                  <i class="el-icon-close" @click="delProperties(idx,item.children,index)"></i>
                </p>
                <el-upload class="avatar-uploader goodsProp" action="http://coopera.xfd365.com/user/auth/uploadImg"
                  :show-file-list="false" :on-success="handlePropSuccess" :on-error="uploadErr" v-if="index==0">
        ...
        <p>商品库存</p>
        <el-table :data="tableData" border style="width: 100%" class="specTable" :span-method="objectSpanMethod"
          :resizable="false" v-if="specs.length != 0">
          ...
          <el-table-column prop="prices.price" label="价格" align="center">
            <template slot-scope="scope">
              <input type="text" class="el-input__inner" v-model="scope.row.prices.price" placeholder="请输入内容"
                @input="scope.row.prices.price = utils.handlePrice(scope.row.prices.price)">
            </template>
          </el-table-column>
          <el-table-column prop="prices.tiered" label="阶梯价格" align="center" v-if="assort_name == '农资商城'">
            <template slot-scope="scope">
              <el-button @click="showAddPrice(scope.$index)" v-if="specPrices[scope.$index].prices.tiered.length == 0">
                添加</el-button>
              <div v-else>
          ...
          <el-table-column prop="prices.cost" label="成本价(元)" align="center">
            <template slot-scope="scope">
              <input type="text" class="el-input__inner" v-model="scope.row.prices.cost" placeholder="请输入内容"
                @input="scope.row.prices.cost = utils.handlePrice(scope.row.prices.cost)">
            </template>
          </el-table-column>
          <el-table-column prop="prices.discount_price" label="会员价(元)" align="center">
            <template slot-scope="scope">
              <input type="text" class="el-input__inner" v-model="scope.row.prices.discount_price" placeholder="请输入内容"
                @input="scope.row.prices.discount_price = utils.handlePrice(scope.row.prices.discount_price)">
            </template>
          </el-table-column>
          <el-table-column v-if="isFenxiao" prop="prices.team_owner_money " label="分销金额" align="center">
            <template slot-scope="scope">
              <input type="text" class="el-input__inner" v-model="scope.row.prices.team_owner_money"
                placeholder="请输入内容">
            </template>
          </el-table-column>
          <el-table-column prop="prices.stock" label="库存" align="center">
            <template slot-scope="scope">
              <input type="text" class="el-input__inner" v-model="scope.row.prices.stock" placeholder="请输入内容"
                @input="scope.row.prices.stock = scope.row.prices.stock.replace(/\D/g,'')">
            </template>
          </el-table-column>
        ...
        <el-table v-if="specs.length != 0" :data="piliangData" border class="piliang" :span-method="objectSpanMethod2">
          ...
          <el-table-column prop="prices.team_owner_money" align="center">
            <template slot-scope="scope">
              <input type="text" class="el-input__inner" v-model="scope.row.prices.team_owner_money" placeholder="请输入内容"
                @input="scope.row.prices.team_owner_money = utils.handlePrice(scope.row.prices.team_owner_money)">
    ...
    <el-dialog title="范例和说明" :visible.sync="isFanli">
      <div style="height:60vh;overflow-y:auto;">
        ...
        <img src="http://ljj-dom.oss-cn-beijing.aliyuncs.com/xfd/spec_add1.png" alt=""
          style="width:100%;margin-bottom:20px;display:block">
    ...
    <el-dialog title="阶梯价格" :visible.sync="isAddPrice">
      <div style="height:60vh;overflow-y:auto;">
        <div class="list" v-for="(item,index) in priceList" :key="'info-'+ index">
          ...
          <p v-else>
            购买数量：{{item.least}}~
            <el-input v-model="item.most"
              @input="inputSecond(item.most,index,item.least);item.most=item.most.replace(/^(0+)|[^\d]+/g,'')"
              type="tel"></el-input> 件
          </p>
          <p>价格：<el-input v-model="item.price" type="tel"
              @input="item.price=item.price.replace(/^\D*(\d*(?:\.\d{0,2})?).*$/g,'$1')"></el-input> 元</p>
          <span @click="addPriceList(index)" v-if="item.addorReduce == '+'">{{item.addorReduce}}</span>
  ...
  export default {
    ...
    data() {
      return {
        ...
        priceList: [], //阶梯列表
        delTieredArr: [], //删除的阶梯价格id数组
        // delTieredArr2:[],//点击确定时存的id，传给后台
        addPriceIndex: 0,
        ...
        this.ajax(
          ...
          res => {
            ...
            if (res.errno == 0) {
              ...
              this.ruleForm.fahuo_date = [res.data.deliver_start, res.data.deliver_end]
              // this.ruleForm.production_date = this.$timestampToTime(
              //   this.ruleForm.production_date
              // );
              if (this.ruleForm.production_date) {
                this.ruleForm.production_date = this.utils.dateFormat('yyyy-MM-dd', new Date(this.ruleForm
                  .production_date * 1000))
              } else {
                this.ruleForm.production_date = ""
              }
              // if(this.ruleForm.booking_date) {
              //   this.ruleForm.booking_date = this.utils.dateFormat('yyyy-MM-dd',new Date(this.ruleForm.booking_date*1000))
              // } else {
              //   this.ruleForm.booking_date = ""
              // }
    ...
    updated() {
      ...
      if (that.initUe) { // 初始化编辑器 (若已经初始化,禁止再次渲染)
        if (document.querySelector('#editor-con')) {
          document.querySelector('#editor-con').innerHTML = '<div  id="' + this.editorId +
            '" style="width:100%;height:300px"></div>'
          setTimeout(() => {
            UE.getEditor(that.editorId, {
              initialFrameHeight: 300,
              initialFrameWidth: document.querySelector('.el-form').clientWidth
            }).ready(function () {
              //this是当前创建的编辑器实例
    ...
    methods: {
      ...
      reducePriceList(index) {
        let str = this.priceList[index].tiered_id
        this.delTieredArr.push(str)
        this.delTieredArr = this.unique(this.delTieredArr)
        this.priceList.splice(index, 1);
        this.priceList[index].least = Number(this.priceList[index - 1].most) + 1
      },
      ...
      // 店铺分类
      cateList() {
        let cooperationInfo = localStorage.getItem('cooperationInfo') ? JSON.parse(localStorage.getItem(
          'cooperationInfo')) : ''
        this.ajax(
          "cateList", {
      ...
      beforeAvatarUpload(file) {
        ...
        return new Promise(resolve => {
          ...
          image.onload = (imageEvent) => {
            let isLt2M = file.size / 1024 / 1024 < 5 // 判定图片大小是否小于5MB
            let imgQuality = 0.1
            if (isLt2M) {
              imgQuality = 0.5
            }
            ...
            const blobData = _this.dataURItoBlob(dataUrl, file.type, file.name);
            resolve(blobData)
          }
          reader.onload = (e => {
          ...
          res => {
            ...
            if (res.errno == 0) {
              ...
              if (res.data.thumb) {
                ...
                document.querySelector('.thumb-icon').parentNode.style.display = 'blok'
              }
              if (res.data.album) {
                let tmpAlbum = res.data.album.split(',')
                tmpAlbum.forEach((item, index) => {
      ...
      handleParams(data) {
        ...
        params.real_name = this.shimingRadio //0否1是
        params.is_team_tg = this.fenxiaoRadio //0否1是
        ...
        params.check_status = 1 // 0-需要审核 1-不需要审核
        params.content = content
        params.tiered_del = JSON.parse(JSON.stringify(this.delTieredArr))
      ...
      submitForm(formName) {
        ...
        if (this.$route.query.goods_id) {
          for (let i = 0; i < tmpData.spcifi.length; i++) {
            ...
            if (!tmpData.spcifi[i].retail_price && tmpData.spcifi[i].is_del == 0) {
              this.$message.error('价格不能为空')
              return
              break;
            }
            if (!tmpData.spcifi[i].discount_price && tmpData.spcifi[i].is_del == 0) {
              this.$message.error('指导价不能为空')
              return
              break;
            }
          }
          ...
          if (this.specs.length == 2) {
            ...
            if (columnIndex === 0) {
              if (
                ...
                return {
                  rowspan: this.specs[1].children.length * this.specs[2].children.length,
                  colspan: 1
                };
              } else {
                return {
      ...
      addNewItem() {
        ...
        this.tableData.forEach(ele => {
          obj = {
            ...
            threeStylesTitle: this.specs[2] != undefined ? this.specs[2].type : "", //规格3标题
            ...
            discount_price: ele.prices.discount_price, //规格会员价/厂商指导价
