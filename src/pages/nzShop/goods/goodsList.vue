    ...
    <div class="operate-con clearfix">
      ...
      <div class="btn-con" v-if="!loading && isAdd">
        <el-button type="primary" size="small"
          @click="$router.push({path: '/nzAddGoods',query: {lastPath: '/nzGoodsList'}})">添加商品</el-button>
      </div>
    </div>
    <div class="table-con">
      <div>
        <el-table :data="tableData" v-if="tableData && tableData.length > 0" border>
          ...
          <el-table-column label="排序" align="center" v-if="isSort">
            <template slot-scope="scope">
              <el-input v-model="scope.row.goods_sort" @blur="inputSort(scope.row.goods_id,scope.row.goods_sort)">
              </el-input>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="log_date" label="操作" align="center" width="160">
            <template slot-scope="scope">
              ...
              <el-button type="danger" plain size="small" v-if="isDel" @click="del(scope.row.goods_id)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
...
<script>
  export default {
    ...
    activated() {
      ...
      if (isRefresh && isRefresh == 1 && tag == 'edit') {
        this.tableData = [];
        ...
        this.ajax("goodsSort", {
          ...
          if (res.errno == 0) {
            ...
            this.tableData.sort(function (a, b) {
              return b.goods_sort - a.goods_sort
            })
          }
        }, err => {})
      },
