    ...
    <div class="operate-con clearfix">
        ...
        <el-form class="clearfix" label-width="80px">
          ...
          <el-form-item label="审核状态">
            <el-select v-model="check_status" placeholder="请选择审核状态">
              <el-option v-for="(item,index) in checkOptions" :key="index" :label="item.name" :value="item.id">
              </el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="分销商品">
            <el-select v-model="is_team_tg" @change="currentSel" placeholder="请选择分销商品">
              <el-option v-for="(item,index) in fxoptions" :key="index" :label="item.name" :value="item.name">
              </el-option>
            </el-select>
          </el-form-item>
      ...
      <div class="btn-con" v-if="!loading && isAdd">
        <el-button type="primary" size="small"
          @click="$router.push({path: '/cooperateAddGoods',query: {lastPath: '/cooperateGoodsList'}})">添加商品</el-button>
      </div>
    </div>
    <div class="table-con">
      <div>
        <el-table :data="tableData" v-if="tableData && tableData.length > 0" border>
          ...
          <el-table-column prop="storage" width="120" label="库存" align="center"></el-table-column>
          <el-table-column prop="retail_price" width="120" label="价格" align="center"></el-table-column>
          ...
          <el-table-column label="审核状态" align="center">
            <template slot-scope="scope">
              ...
              <template v-if="scope.row.check_status == 2">
                <el-popover placement="top-start" title="拒绝原因" trigger="hover" :content="scope.row.note">
                  <el-tag style="cursor: pointer" type="danger" slot="reference">已拒绝</el-tag>
                </el-popover>
              </template>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="log_date" label="操作" align="center" width="160">
            <template slot-scope="scope">
              ...
              <el-button type="danger" plain size="small" @click="del(scope.row.goods_id)" v-if="isDel">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
