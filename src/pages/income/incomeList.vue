        ...
        <div class="breadcrumb-con">
            ...
            <div class="breadcrumb-info">
                <el-breadcrumb separator-class="el-icon-arrow-right">
                    ...
                    <el-breadcrumb-item  class="breadcrumb-tit">收益列表</el-breadcrumb-item>
                </el-breadcrumb>
            </div>
        ...
        <div class="table-con">
            <div>
              <el-table :data="tableData" v-if="tableData && tableData.length > 0" border>
                ...
                <el-table-column prop="coopera_name" label="开户机构" show-overflow-tooltip v-if="isAdmin" align="center" ></el-table-column>
                <el-table-column prop="bonus_num" label="收益账户" align="center"></el-table-column>
                <el-table-column prop="log_type" label="记录类型" align="center">
                    <template slot-scope="scope">
                        <template v-if="scope.row.log_type == 0">
                            <span>活期存款</span>
                        </template>
                        <template v-if="scope.row.log_type == 1">
                            <span>活期取款</span>
                        </template>
                        <template v-if="scope.row.log_type == 2">
                            <span>贷款</span>
                        </template>
                        <template v-if="scope.row.log_type == 3">
                            <span>还款 </span>
                        </template>
                        <template v-if="scope.row.log_type == 4">
                            <span>活期利息增加</span>
                        </template>
                        <template v-if="scope.row.log_type == 5">
                            <span>卡内部转入</span>
                        </template>
                        <template v-if="scope.row.log_type == 6">
                            <span>卡内部转出</span>
                        </template>
                        <template v-if="scope.row.log_type == 7">
                            <span>分红</span>
                        </template>
                        <template v-if="scope.row.log_type == 8">
                            <span>合作社卡转出到银行卡</span>
                        </template>
                        <template v-if="scope.row.log_type == 9">
                            <span>股金存入</span>
                        </template>
                        <template v-if="scope.row.log_type == 10">
                            <span>股金退股</span>
                        </template>
                        <template v-if="scope.row.log_type == 11">
                            <span>股金转入 </span>
                        </template>
                        <template v-if="scope.row.log_type == 12">
                            <span>股金转出</span>
                        </template>
                        <template v-if="scope.row.log_type == 13">
                            <span>收益支取</span>
                        </template>
                        <template v-if="scope.row.log_type == 14">
                            <span>定期存入</span>
                        </template>
                         <template v-if="scope.row.log_type == 15">
                            <span>定期取出</span>
                        </template>
                        <template v-if="scope.row.log_type == 16">
                            <span>购物消费</span>
                        </template>
                        <template v-if="scope.row.log_type == 17">
                            <span>购物退款</span>
                        </template>
                        <template v-if="scope.row.log_type == 18">
                            <span>退款失败</span>
                        </template>
                        <template v-if="scope.row.log_type == 19">
                            <span>定期利息</span>
                        </template>
                    </template>
                </el-table-column>
                <el-table-column prop="last_money" label="变动之前会员卡余额" align="center"></el-table-column>
                <el-table-column prop="log_money" label="记录变动的金额" align="center">
                    <template slot-scope="scope">
                     {{((scope.row.log_type == 4 || scope.row.log_type == 7 || scope.row.log_type == 19)?'+' : '-') +scope.row.log_money}}
                    </template>
                </el-table-column>
                <el-table-column prop="later_money" label="余额" align="center"></el-table-column>
