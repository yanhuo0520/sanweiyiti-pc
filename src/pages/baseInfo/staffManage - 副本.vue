<template>
  <div class="staff-manage" :class="{'no-padding': isAdmin}" v-loading="loading">
          ...
          <div v-for="(item,index) in cooperativeList" :key="index">
            <template v-if="item.count > 0">
              <el-submenu :index="index">
                  ...
                  <el-menu-item v-for="(subItem,subIndex) in item.child" :key="subIndex" :index="index+'-'+subIndex">{{subItem.coopera_name}}</el-menu-item>
              </el-submenu>
            </template>
            <template v-else>
                <el-menu-item :index="index">
                  <span slot="title">{{item.coopera_name}}</span>
                </el-menu-item>
            </template>
          </div>
        </el-menu>
